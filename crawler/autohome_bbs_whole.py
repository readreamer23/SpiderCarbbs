...
def getTitleBBSContent(url):
    ...
    for i in range(1,count):
        sleepsecs=random.random()*7+random.random()*3
...
#start=从第几页开始爬，url=论坛页地址，title=车型
def getTotalBBSContent(start,url,title):
    count=getTitlePageCount(url)
    #count=1
    print(title+'一共多少页='+str(count)+',从第'+str(start)+'页开始爬')
    #for i in range(count):
    for i in range(start,count):
        sleepsecs=random.random()*5+random.random()*9
        parent_index=i
        print('爬取'+title+'论坛第'+str(i)+'页')
        print('爬取下个页面前sleep'+str(sleepsecs)+'秒')
        time.sleep(sleepsecs)
        ...
        for index,i in enumerate(titleItemList):
            print('爬取'+title+'论坛大标题第'+str(parent_index)+'页')
...
if __name__=='__main__':
    ...
    #url='https://club.autohome.com.cn/bbs/forum-c-4080-189.html?qaType=-1#pvareaid=101061'
    
    title='长安CS35 PLUS'
    url='https://club.autohome.com.cn/bbs/forum-c-4976-1.html?qaType=-1#pvareaid=101061'
    start=70    
         
    getTotalBBSContent(start,url,title)
    browser.close()
